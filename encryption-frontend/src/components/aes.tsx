  ...
  const handleAesKeyEnter = (value: string) => {
    ...
    if (value.startsWith("0x")) {
      ...
      for (let c of value.slice(2)) {
        if (!valid.includes(c)) {
          setAesKeyInputStatus(
            "Character '" + c + "' is invalid for hexadecimal input"
          );
          return;
        } else if (c != " " && c != "_" && c != "-" && c != "\n") {
          ++len;
      ...
      for (let c of value.slice(2)) {
        if (!valid.includes(c)) {
          setAesKeyInputStatus(
            "Character '" + c + "' is invalid for binary input"
          );
          return;
        } else if (c != " " && c != "_" && c != "-" && c != "\n") {
          ++len;
      ...
      setAesKeyInputStatus(
        "Key must start with 0b (for binary) or 0x (for hexadecimal)"
      );
      return;
    }
    if (value.startsWith("0x")) {
      if (len < aesKeySize / 4) {
        ...
        setAesKeyWarning(false);
        setAesKeyInputStatus(
          "Key cannot exceed " + aesKeySize.toString() + " bits"
        );
      }
    } else if (value.startsWith("0b")) {
      if (len < aesKeySize) {
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-2">
        <Label htmlFor="aes-type" className="block">
          AES Key Size
        </Label>
        <select
      ...
      <div className="space-y-2">
        <Label htmlFor="aes-mode" className="block">
          AES Block Mode
        </Label>
        <select
          className="bg-muted px-2 py-2 outline"
      ...
      <div className="space-y-2">
        <Label htmlFor="aes-datatype" className="block">
          Input Data Type
        </Label>
        <select
          className="bg-muted px-2 py-2 outline"
        ...
        {aesKeyWarning && !aesKeyInputStatus && (
          <div>
            <p className="text-yellow-700 text-sm">
              Keys under {aesKeySize} bits will be left-padded with zeros
            </p>
          </div>
        )}
      </div>
      <div className="space-y-2">
        ...
        {aesDataType === "file" ? (
          ...
          <TextArea
            ...
            className={
              aesDataType == "binary" && aesData.length > 0 ? "font-mono" : ""
            }
