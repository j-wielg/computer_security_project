...
// Lookup tables for multiplying in the finite field of order 8
// Values were found at 'https://en.wikipedia.org/wiki/Rijndael_MixColumns'
const MUL2 = [
...
export function aesBlockEncrypt(block: Array<number>, key: bigint, size: number, W: number[] = []) {
    ...
    const shiftRows = () => {
        // For this, we treat the 16 bytes as a 4x4 matrix
        // We ignore the first row
        // The second row is shifted by 1
        let temp = block[1];
        block[1] = block[5];
        block[5] = block[9];
        block[9] = block[13];
        block[13] = temp;
        // The third row is shifted by 2
        temp = block[2];
        block[2] = block[10];
        block[10] = temp;
        temp = block[6];
        block[6] = block[14];
        block[14] = temp;
        // The fourth row is shifted by 3, which is equivalent to a right shift of 1
        temp = block[15];
        block[15] = block[11];
        block[11] = block[7];
        block[7] = block[3];
    ...
    const mixColumns = () => {
        ...
        for (let col = 0; col < 4; ++col) {
            temp[0] = block[4 * col];
            temp[1] = block[4 * col + 1];
            temp[2] = block[4 * col + 2];
            temp[3] = block[4 * col + 3];
            block[4 * col] = MUL2[temp[0]] ^ MUL3[temp[1]] ^ temp[2] ^ temp[3];
            block[4 * col + 1] = temp[0] ^ MUL2[temp[1]] ^ MUL3[temp[2]] ^ temp[3];
            block[4 * col + 2] = temp[0] ^ temp[1] ^ MUL2[temp[2]] ^ MUL3[temp[3]];
            block[4 * col + 3] = MUL3[temp[0]] ^ temp[1] ^ temp[2] ^ MUL2[temp[3]];
        }
    }
    ...
    for (let i=0; i < rounds; ++i) {
        // Substep 1: Do the SBOX substitution
        for (let j=0; j < 16; ++j) {
            block[j] = aesSBox(block[j]);
        }
        // Substep 2: Shift rows
        shiftRows();
        // Substep 3: Mix the columns
        mixColumns();
...
/**
 * Implements the AES S-Box
 */
function aesSBox(byte: number) : number {
    let row = (byte & 0xF0) >> 4;
...
/**
 * Implements the inverse AES S-box
 */
function aesInvSBox(byte: number) : number {
    let row = (byte & 0xF0) >> 4;
    let column = byte & 0x0F;
 ...
 */
export function aesBlockDecrypt(block: Array<number>, key: bigint, size: number, W: number[] = []) {
    const addRoundKey = (index: number) => {
        for (let i=0; i < 4; ++i) {
            let word = W[index * 4 + i];
            let mask = 0xFF000000;
    ...
    const invShiftRows = () => {
        // For this, we treat the 16 bytes as a 4x4 matrix
        // We ignore the first row
        // The second row is shifted right by 1
        let temp = block[13];
        block[13] = block[9];
        block[9] = block[5];
        block[5] = block[1];
        ...
        block[14] = block[6];
        block[6] = temp;
        // The third row is shifted left by 1
        temp = block[3];
        block[3] = block[7];
        block[7] = block[11];
        block[11] = block[15];
    ...
    const invMixCols = () => {
        ...
        for (let col = 0; col < 4; ++col) {
            ...
            temp[3] = block[4 * col + 3];
            block[4 * col] = (
                MUL14[temp[0]] ^ MUL11[temp[1]] ^ MUL13[temp[2]] ^ MUL09[temp[3]]
            );
            block[4 * col + 1] = (
                MUL09[temp[0]] ^ MUL14[temp[1]] ^ MUL11[temp[2]] ^ MUL13[temp[3]]
            );
            block[4 * col + 2] = (
                MUL13[temp[0]] ^ MUL09[temp[1]] ^ MUL14[temp[2]] ^ MUL11[temp[3]]
            );
            block[4 * col + 3] = (
                MUL11[temp[0]] ^ MUL13[temp[1]] ^ MUL09[temp[2]] ^ MUL14[temp[3]]
            );
        }
    }
    // Step 1: Generation of round keys
